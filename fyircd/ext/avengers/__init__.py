...
def _on_load():
	...
	add_fake_user({'nickname': 'Loki', 'realname': 'Loki', 'hostname': 'Loki.fy.to', 'oper': True})

	add_user_privmsg_callback('Loki'.lower(), _operserv)
	add_user_privmsg_callback('ChanServ'.lower(), _chanserv)
	add_user_privmsg_callback('NickServ'.lower(), _nickserv)

	for bot in bot_list:
		add_fake_user({'nickname': bot, 'realname': bot, 'hostname': 'avengers.fy.to', 'oper': True})
...
def _operserv(target, params):
	...
	if target.oper == False:
		target.write(
			':%s %s %s :%s' % (
				operserv, 'NOTICE', target.nickname,
				'Enough! You are, all of you are beneath me! I am a god, you dull creature, and I will not be bullied ... YOU\'RE NOT A GOD HERE!'
			)
		)
	elif args[0] == 'initdb':
		...
		for user in target.server.users.values():
			...
			user.write(
				':%s %s %s :%s' % (
					operserv, 'NOTICE', target.nickname,
					'Service Database has been wiped - please use /msg nickserv help ^_^'
				)
			)


def _chanserv(target, params):
	...
	if args[0] == 'register':
		if len(args) == 2:
			...
			if not channel:
				target.write(
					':%s %s %s :%s' % (
						chanserv, 'NOTICE', target.nickname,
						'Nope --- Register: You obviously need to be on the channel.'
					)
				)
			if target not in channel.modes.data['q']:
				target.write(
					':%s %s %s :%s' % (
						chanserv, 'NOTICE', target.nickname,
						'Nope --- Register: You\'re not the owner of this channel. But nice try. Dream big!'
					)
				)
			else:
				...
				if exist:
					target.write(
						':%s %s %s :%s' % (
							chanserv, 'NOTICE', target.nickname,
							'Nope --- Register: This channel is already registered.'
						)
					)
				else:
					if target.modes.data['r'] == 1 and target.service_user != None:
						chan = AvengerChan(
							id=channel.id, name=channel.name, id_owner=target.service_user.nick, bot=None
						)
						session.add(chan)
						session.commit()
						...
						target.write(
							':%s %s %s :%s' % (
								chanserv, 'NOTICE', target.nickname,
								'Nope --- Register: You need to be logged with NickServ.'
							)
						)
		else:
			target.write(
				':%s %s %s :%s' %
				(nickserv, 'NOTICE', target.nickname, 'Register: /msg chanserv register <chan>')
			)
		...
		if len(args) == 3:
			...
			if not channel:
				target.write(
					':%s %s %s :%s' % (
						chanserv, 'NOTICE', target.nickname,
						'Nope --- Assign: You obviously need to be on the channel.'
					)
				)
			else:
				...
				if not exist:
					target.write(
						':%s %s %s :%s' % (
							chanserv, 'NOTICE', target.nickname,
							'Nope --- Assign: You need to register this channel.'
						)
					)
					...
					target.write(
						':%s %s %s :%s' % (
							chanserv, 'NOTICE', target.nickname,
							'Nope --- Assign: You are not the owner of this channel. IDENTIFY YOURSELF!'
						)
					)
				else:
					if args[2].lower() in bot_list_lower:
						...
						target.write(
							':%s %s %s :%s' % (
								chanserv, 'NOTICE', target.nickname,
								'Nope --- Assign: These aren\'t the droids you\'re looking for - /msg chanserv bots'
							)
						)
		else:
			target.write(
				':%s %s %s :%s' %
				(chanserv, 'NOTICE', target.nickname, 'Assign a bot: /msg chanserv <chan> <bot_name>')
			)
			target.write(
				':%s %s %s :%s' %
				(chanserv, 'NOTICE', target.nickname, 'List all bots: /msg chanserv bots')
			)
	elif args[0] == 'bots':
		str_bot_list = ', '.join(bot_list)
		print(str_bot_list)
		target.write(
			':%s %s %s :%s' %
			(chanserv, 'NOTICE', target.nickname, 'Bots: %s' % (str_bot_list))
		)
		...
		target.write(':%s %s %s :%s' % (chanserv, 'NOTICE', target.nickname, 'Help with NickServ: '))
		target.write(
			':%s %s %s :%s' %
			(chanserv, 'NOTICE', target.nickname, 'Register: /msg nickserv register <pass> <mail> ')
		)
		target.write(
			':%s %s %s :%s' %
			(chanserv, 'NOTICE', target.nickname, 'Identify: /msg nickserv identify <pass> ')
		)
		...
		target.write(':%s %s %s :%s' % (chanserv, 'NOTICE', target.nickname, 'Help  with ChanServ: '))
		target.write(
			':%s %s %s :%s' %
			(chanserv, 'NOTICE', target.nickname, 'Register: /msg chanserv register <chan>')
		)
		target.write(
			':%s %s %s :%s' %
			(chanserv, 'NOTICE', target.nickname, 'Assign a bot: /msg chanserv <chan> <bot_name>')
		...
		target.write(
			':%s %s %s :%s' % (chanserv, 'NOTICE', target.nickname, 'List all bots: /msg chanserv bots')
		)
		...
		target.write(
			':%s %s %s :%s' %
			(chanserv, 'NOTICE', target.nickname, '? Donde esta la biblioteca ? (try /msg ChanServ help)')
		)


def _nickserv(target, params):
	...
	if args[0] == 'register':
		if len(args) != 3:
			target.write(
				':%s %s %s :%s' % (
					nickserv, 'NOTICE', target.nickname,
					'Nope --- Register: /msg nickserv register <pass> <mail> '
				)
			)
		else:
			...
			if exist:
				target.write(
					':%s %s %s :%s' %
					(nickserv, 'NOTICE', target.nickname, '.... This nickname is already registered.')
				)
			else:
				...
				target.write(
					':%s %s %s :%s' % (
						nickserv, 'NOTICE', target.nickname,
						'GG, Welcome!. Now use /msg nickserv identify <pass>.'
					)
				)
				session.commit()
		...
		if len(args) != 2:
			target.write(
				':%s %s %s :%s' %
				(nickserv, 'NOTICE', target.nickname, 'Nope --- Register: /msg nickserv identify <pass> ')
			)
		else:
			user = session.query(AvengerUser).filter(AvengerUser.nick == target.nickname.lower()).first()
			if user and check_password_hash(user.password, args[1]):
				target.write(
					':%s %s %s :%s' % (
						nickserv, 'NOTICE', target.nickname,
				...
				target.write(
					':%s %s %s :%s' %
					(nickserv, 'NOTICE', target.nickname, 'Nice try ... (incorrect password)')
				)
		...
		target.write(
			':%s %s %s :%s' %
			(nickserv, 'NOTICE', target.nickname, 'Register: /msg nickserv register <pass> <mail> ')
		)
		target.write(
			':%s %s %s :%s' %
			(nickserv, 'NOTICE', target.nickname, 'Identify: /msg nickserv identify <pass> ')
		)
		...
		target.write(
			':%s %s %s :%s' %
			(nickserv, 'NOTICE', target.nickname, 'Assign a bot: /msg chanserv <chan> <bot_name>')
		)
		target.write(
			':%s %s %s :%s' % (nickserv, 'NOTICE', target.nickname, 'List all bots: /msg chanserv bots')
		)
		...
		target.write(
			':%s %s %s :%s' %
			(nickserv, 'NOTICE', target.nickname, '? Donde esta la biblioteca ? (try /msg NickServ help)')
		)
